...
const HeroSlider = () => {
    ...
    const fetchSlides = async () => {
      ...
      try {
        const {
          data,
          error
        } = await supabase.from('slides').select('*').order('order_position', {
          ascending: true
        });
  ...
  if (loading) {
    return <div className="py-6 overflow-hidden bg-flyboy-dark">
      ...
      <div className="container">
        <div className="relative w-full h-[60vh] md:h-[70vh] mx-auto rounded-2xl overflow-hidden shadow-2xl">
          {slides.map((slide, index) => <div key={slide.id} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
            ...
            backgroundImage: `url(${slide.image_url})`
              ...
              <div className="absolute bottom-0 left-0 right-0 h-12 flex items-end justify-center space-x-1 px-4">
                {Array(20).fill(0).map((_, i) => <div key={i} className="w-1 bg-flyboy-gold opacity-70" style={{
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center">
                ...
                {slide.subtitle && <p className="text-xl md:text-2xl max-w-2xl px-4 animate-fade-in mb-8">
                    {slide.subtitle}
                  </p>}
                
                <div className="flex items-center justify-center space-x-4">
                  <button onClick={togglePause} className="bg-flyboy-gold bg-opacity-70 hover:bg-opacity-100 p-3 rounded-full transition-all duration-300 transform hover:scale-110">
          ...
          <div className="absolute bottom-5 left-1/2 transform -translate-x-1/2 flex space-x-2">
            {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-flyboy-gold w-6' : 'bg-white bg-opacity-50'}`} aria-label={`انتقل إلى الشريحة ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </div>;
