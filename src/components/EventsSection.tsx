...
const EventsSection = () => {
  ...
  useEffect(() => {
    if (events.length > 0) {
      ...
      const allKeywords = events.map(event => event.keywords).filter(Boolean).join(', ');
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="p-0 border-none w-full max-w-none sm:max-w-none md:max-w-none lg:max-w-none bg-transparent shadow-none">
          <div className="relative bg-black/95 w-full h-[90vh] md:h-[90vh] flex items-center justify-center">
            {selectedEvent && <div className="relative w-full h-full">
                <img src={selectedEvent.image_url} alt={selectedEvent.title} className="w-full h-full object-contain" />
              </div>}
            <button className="absolute top-4 right-4 bg-black/70 text-white p-2 rounded-full hover:bg-black/90 transition-colors z-10" onClick={() => setDialogOpen(false)}>
              <X className="h-6 w-6" />
            </button>
          </div>
        </DialogContent>
