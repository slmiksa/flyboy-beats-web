...
const AdminSocialMedia = () => {
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (editingLink) {
        // تحديث رابط موجود
        result = await supabase.from('social_media').update({
        ...
        // إنشاء رابط جديد
        result = await supabase.from('social_media').insert({
          platform: values.platform,
          url: values.url,
          icon: values.icon || values.platform,
      ...
      toast({
        title: "تم بنجاح",
        description: editingLink ? "تم تحديث رابط التواصل الاجتماعي بنجاح" : "تم إضافة رابط التواصل الاجتماعي بنجاح"
      });
    } catch (error: any) {
      toast({
        title: "خطأ",
  ...
  const moveOrder = async (link: SocialMedia, direction: 'up' | 'down') => {
    try {
      ...
      const {
        error: error1
      } = await supabase.from('social_media').update({
        order_position: targetLink.order_position
      }).eq('id', link.id);
      const {
        error: error2
      } = await supabase.from('social_media').update({
  ...
  return <div className="space-y-6">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="flex justify-end pt-4">
                  <Button type="submit" className="bg-flyboy-gold hover:bg-flyboy-gold/80 text-flyboy-dark" disabled={isSubmitting}>
                    {isSubmitting ? <>
      ...
      <div className="grid gap-6">
        {isLoading ? <Card className="border-flyboy-gold/30 bg-flyboy-purple/30">
            <CardContent className="flex justify-center items-center py-10">
              <Loader2 className="h-8 w-8 animate-spin text-flyboy-gold" />
            </CardContent>
          </Card> : socialLinks.length === 0 ? <Card className="border-flyboy-gold/30 bg-flyboy-purple/30">
            <CardContent className="flex flex-col items-center py-10 space-y-4">
              ...
              <Button className="bg-flyboy-gold hover:bg-flyboy-gold/80 text-flyboy-dark" onClick={() => setIsDialogOpen(true)}>
                <Plus className="h-4 w-4 mr-2" />
                إضافة رابط جديد
              </Button>
            </CardContent>
          </Card> : <Card className="border-flyboy-gold/30 bg-flyboy-purple/30">
            <CardHeader>
              <CardTitle className="text-xl text-flyboy-gold">قائمة روابط التواصل الاجتماعي</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {socialLinks.map(link => <div key={link.id} className="flex items-center justify-between bg-flyboy-dark/40 p-3 rounded-md">
                    ...
                    <div className="flex items-center space-x-2 rtl:space-x-reverse">
                      <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => moveOrder(link, 'up')}>
                        <ArrowUp className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => moveOrder(link, 'down')}>
                        <ArrowDown className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={() => handleEdit(link)}>
                        <Edit2 className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="destructive" className="h-8 w-8 p-0" onClick={() => handleDelete(link.id)}>
